...
public class FileStorageService {
    ...
    public FileResponse upload(MultipartFile file) {  
        if (file == null || file.isEmpty()) {   //проверяет что файл не пустой
            throw new IllegalArgumentException("File is empty");
        }
        if (file.getSize() > MAX_FILE_SIZE_BYTES) {   //проверяет что файл не превышает ограничение в 10мб
            throw new IllegalArgumentException("File size exceeds 10MB limit");
        }

        String originalFileName = StringUtils.cleanPath(   //очищает имя файла от потенциально опасных частей пути с помощью StringUtils.cleanPath
                Objects.requireNonNullElse(file.getOriginalFilename(), "file")
        );
        originalFileName = Paths.get(originalFileName).getFileName().toString(); //генерирует уникальное внутреннее имя файла на
        if (originalFileName.isBlank() || originalFileName.contains("..")) {     //основе UUID c сохранением исходного расширения
        ...
        try {
            Files.copy(file.getInputStream(), targetPath, StandardCopyOption.REPLACE_EXISTING); //копирует содержимое файла в каталог 
    ...
    public Resource download(Long id) {  //предоставляет методы скачивания и удаления файла как с диска, так и из базы данных
